...
describe("validateTodo", () => {
    it("check if value is not longer than 255 characters", () => {
        expect(validateTodo({ id: "1", value: "a".repeat(256), done: false }, [])).toEqual(false);
    });
    it("check if value is not empty", () => {
        expect(validateTodo({ id: "1", value: "", done: false }, [])).toEqual(false);
    });
    it("check if value is not empty",()=>{
        expect(validateTodo({ id: "1", value: " ", done: false }, [])).toEqual(false);
    })
    it("check if todo is not contained in the todos array (case insensitive)", () => {
        expect(validateTodo({ id: "1", value: "a", done: false }, [{ id: "2", value: "A", done: false }])).toEqual(false);
    });
});
...
describe("formatTodo", () => {
    ...
    it("check if first letter is capitalized", () => {
        expect(formatTodo({ id: "1", value: "Abb", done: false }).value).toEqual("Abb");
    });
});
